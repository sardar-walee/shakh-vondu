...
import { 
  ...
  batchImportCustomersToFirestore, 
  ParsedImportCustomer 
...
export default function CustomerCsvImportModal({ isOpen, onClose, storeId, onSuccess }: CustomerCsvImportModalProps) {
  const [file, setFile] = useState<File | null>(null);
  const [parsedItems, setParsedItems] = useState<ParsedImportCustomer[]>([]);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      const items = parseCustomerCSV(text);
  ...
  const handleStartImport = async () => {
    ...
    try {
      const result = await batchImportCustomersToFirestore(storeId, parsedItems, (count, total) => {
      ...
      >
        ...
        <div className="p-6 bg-gradient-to-r from-slate-900 to-indigo-950 text-white flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-black text-white leading-tight">Import Customer Directory (CSV / Excel)</h3>
              <p className="text-xs text-indigo-200 font-medium mt-0.5">Import customer contact lists, outstanding debt balances, and loyalty points.</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto space-y-6 custom-scrollbar flex-1">
          ...
          <div className="bg-indigo-50/80 border border-indigo-200 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="text-xs font-black text-indigo-900 uppercase tracking-wider">Download Customer Import Template</h4>
                <p className="text-xs text-indigo-800 mt-0.5">Includes sample format for Name, Phone, Address, Debt Balance, and Points.</p>
              </div>
            </div>
            <button
              onClick={downloadCustomerSampleCSV}
              className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs rounded-xl shadow-md transition-all flex items-center gap-2 cursor-pointer whitespace-nowrap"
            >
          ...
          {/* Upload Area */}
          {!importResult && (
            <div className="border-2 border-dashed border-gray-300 hover:border-indigo-500 rounded-3xl p-8 text-center transition-all bg-gray-50/50 hover:bg-indigo-50/30">
              <input
                type="file"
                accept=".csv, .txt, .tsv"
                onChange={handleFileChange}
                className="hidden"
                id="customer-csv-upload-input"
              />
              <label 
                htmlFor="customer-csv-upload-input"
                className="cursor-pointer flex flex-col items-center justify-center space-y-3"
              >
                <div className="w-14 h-14 bg-indigo-100 text-indigo-600 rounded-2xl flex items-center justify-center shadow-md">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">Supports CSV or TSV exports from Excel or Contacts</p>
                </div>
              </label>
            </div>
          )}
                ...
                <table className="w-full text-left text-xs">
                  <thead className="bg-gray-100 font-bold text-gray-700 uppercase text-[10px] sticky top-0">
                    <tr>
                      ...
                      <th className="p-3">Debt Balance</th>
                  ...
                  <tbody className="divide-y divide-gray-100 font-medium">
                    {parsedItems.map((item, idx) => (
                      <tr key={idx} className={item.isValid ? 'hover:bg-gray-50' : 'bg-amber-50/50'}>
                        ...
                        <td className="p-3 font-bold text-red-600">${item.debt}</td>
                        <td className="p-3 font-bold text-purple-600">{item.loyaltyPoints} pts</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {importResult && (
            <div className="bg-emerald-50 border border-emerald-200 rounded-3xl p-6 text-center space-y-3">
              ...
              <h4 className="text-base font-black text-emerald-900">Customers Directory Imported Successfully!</h4>
              <p className="text-xs text-emerald-800">
                Added <span className="font-bold">{importResult.successCount}</span> customers to your store database.
                {importResult.failCount > 0 && ` (${importResult.failCount} skipped due to invalid fields)`}
              </p>
              <button
                onClick={onClose}
        ...
        {!importResult && (
          <div className="p-4 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
            ...
            <button
              ...
              className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-black text-xs uppercase tracking-wider rounded-xl shadow-lg transition-all flex items-center gap-2 cursor-pointer"
            >
              <Users className="w-4 h-4" />
              <span>{isImporting ? 'Importing...' : `Import ${validCount} Customers`}</span>
            </button>
          </div>
        )}
      </motion.div>
